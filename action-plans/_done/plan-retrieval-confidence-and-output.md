...
- 권장 시작값: 복합 점수 도메인 기준 `1.0`-`3.0` (0-15 범위에서 하위 약 7-20%, 약한 매칭 tail 대상. 로깅 인프라 구축 후 데이터 기반 조정)
...
- **Deep Analysis 최종 반영 -- Cluster Tautology 수학적 증명:**
  - 원안 임계치(`>= 3`)는 `max_inject=3`(기본값)에서 **대다수(70%+)의 성공적인 쿼리**에 발동하는 논리 오류 (tautology). 잘린 후 결과 N ≤ max_inject이므로 C ≤ N ≤ max_inject → `C > max_inject`는 **수학적으로 불가능** (dead code).
  ...
  - **기본값:** `cluster_detection_enabled: false` (V2-fresh + V2-adversarial + Deep Analysis 합의)
- **로깅 인프라 연계**: 클러스터 감지 발동 시 logging 이벤트 기록 (Plan #2) -> false demotion 데이터 수집 -> 데이터 기반 활성화/임계치 결정

**핵심 수정 (V2-adversarial → Deep Analysis 최종 반영): 복합 점수 도메인 유지**

> **구현 결론:** `confidence_label()`은 복합 점수(BM25 - body_bonus)를 그대로 사용한다 (코드 변경 없음). 아래는 이 결정에 이른 검토 이력이다.
>
> **V2-adversarial 발견 -- Score Domain Paradox:** 현재 `_output_results()`에서 `confidence_label()`에 전달되는 `score`는 `BM25 - body_bonus` 변이값(복합 점수). `body_bonus`는 본문 토큰 매칭 보너스(0-3점, `score_with_body()` line 247에서 계산)이며 순수 BM25 점수가 아님. `score_with_body()` (line 257)에서 `r["score"] = r["score"] - body_bonus`로 in-place 변이됨. `raw_bm25` 필드가 line 256에서 이미 계산/보존되지만 하류에서 소비되지 않음.
>
> **초기 제안 (기각됨):** `confidence_label()`에 `raw_bm25`를 전달하여 순수 BM25 품질로 신뢰도 분류.
>
> **Deep Analysis 결과 -- Ranking-Label Inversion (NEW-4, HIGH):** `raw_bm25` 사용 시 **랭킹-라벨 역전** 발생. 예: Entry A (raw_bm25=-1.0, body_bonus=3, composite=-4.0, 랭킹 #1)는 "low" 라벨, Entry B (raw_bm25=-3.5, composite=-3.5, 랭킹 #2)는 "high" 라벨. Action #2 계층적 출력에서 LOW=침묵이므로, **#1 랭킹 결과가 침묵되고 #2 결과만 주입되는 기능적 버그** 발생.
>
> **최종 결정:** `confidence_label()`은 **복합 점수(BM25 - body_bonus)를 그대로 사용** (현재 코드 유지, 코드 변경 없음). 이는 의도적 설계: 라벨이 Action #2에서 행동 트리거(침묵/축약/전체)로 사용되므로, 라벨은 반드시 랭킹과 단조 관계를 유지해야 함.
>
> **`abs_floor` 교정:** 복합 점수 도메인 기준으로 교정 (일반 코퍼스에서 대략 0-15 범위). `raw_bm25`는 line 256에서 보존되며, 진단 로깅(Plan #2)에만 사용.
>
...
```
- `score`, `best_score`: **복합 점수 사용** (현행 `entry.get("score", 0)` 그대로 -- 코드 변경 없음)
- `abs_floor`: 절대 하한선. `abs(best_score) < abs_floor`이면 "high" 불가. **복합 점수 도메인 기준 교정** (일반 코퍼스에서 권장 시작값 1.0-3.0)
- `cluster_count`: ratio > 0.90인 결과 수 (복합 점수 기반 ratio). **현재 비활성** -- 호출 시 항상 `0` 전달. 향후 활성화 시 pre-truncation counting으로 구현 필요 (Deep Analysis: post-truncation counting은 dead code, `temp/41-finding2-cluster-logic.md` 참조).

**호출 지점 변경:**
- `_output_results()` (line 299): `confidence_label()` 호출 시 `abs_floor=abs_floor`, `cluster_count=0` 전달 (비활성 -- 향후 활성화 시 pre-truncation counting 구현 필요)
- 설정 파싱: `main()` (line 353-384 영역)에서 `abs_floor` 읽기 추가

**`apply_threshold()`와의 상호작용 (개발자 참고):**
`apply_threshold()` (selection)과 `confidence_label()` (labeling)은 독립적 임계치. `apply_threshold()`는 25% noise floor로 결과를 필터링한 후, `confidence_label()`은 남은 결과에 대해 분류한다. 두 임계치가 조율되지 않으므로 `apply_threshold()`를 통과한 클러스터 결과가 `confidence_label()`에서 모두 "high"가 되는 것이 현재 문제이다. 클러스터 감지는 보류되지만, `abs_floor`를 도입하여 점수 미달 결과가 "high"로 분류되는 오분류를 우선 방어한다.
...
- **신규 테스트 필요**: ~10-12개
  - abs_floor 경계값 (0.0, 양수, BM25 음수 점수) (~8개)
  - cluster_count=0 기본값 시 기존 동작 유지 확인 (~2개, 비활성 기능 회귀 방지)
...
MEDIUM 결과 존재 시 검색 유도 문구 (`_emit_search_hint("medium_present")` 호출, 분기 조건: `not any(conf == "high") and any(conf == "medium")`):
```xml
<memory-note>Some results had medium confidence. Use /memory:search &lt;topic&gt; for detailed lookup.</memory-note>
```

**"모든 결과가 LOW"인 경우:**
tiered 모드에서 모든 결과가 LOW일 경우, `_output_results()`는 `<memory-context>` 래퍼를 출력하지 않고 `_emit_search_hint("all_low")`만 호출하여 빈 래퍼 주입을 방지한다. Action #3의 all-low-confidence hint가 이 경로를 처리.
...
| `assets/memory-config.default.json` | `confidence_abs_floor`, `cluster_detection_enabled`, `output_mode` 추가 | #1, #2 |
...
5. **Gate E** (Action #4 후): Agent hook이 `feat/agent-hook-poc` 브랜치에서 정상 로드되고, 4가지 핵심 질문(레이턴시, 컨텍스트 주입 메커니즘, Plugin 호환성, 하이브리드 연쇄)에 대한 답변이 `temp/agent-hook-poc-results.md`에 문서화됨
...
**리뷰 반영 합의 (2차):** 클러스터 감지 설정 토글 추가 (`cluster_detection_enabled`, default: `false`).  abs_floor 코퍼스 의존성 경고 추가. cluster_count 의미론 문서화.
...
| 검토 | 결과 | 핵심 발견 |
|------|------|----------|
| Engineering Review | APPROVE WITH CHANGES | logging.enabled default, schema_version, abs_floor warning |
| Adversarial Review | APPROVE WITH CHANGES | Privacy residue, cluster toggle, session_id gap |
| V1-Robustness | PASS WITH NOTES | Cluster detection config, rollback count correction |
| V1-Practical | PASS | LOC estimates verified, all code references exact |
| V2-Fresh Eyes | NEEDS WORK → fixed | Cluster detection + max_inject=3 interaction is actively harmful |
| V2-Adversarial | 2 CRITICAL → fixed | Score Domain Paradox (raw_bm25), Cluster Tautology (> max_inject) |
| Deep Analysis (7-agent) | raw_bm25 code change REJECTED | Ranking-label inversion (NEW-4) breaks tiered output. Composite score is intentional. Cluster tautology proven dead code. Import hardening (Finding #5, ~36 LOC): module-level try/except + e.name scoping for memory_logger/judge imports. |

**미추적 구현 사항:** Deep Analysis Finding #5 (import hardening, ~36 LOC)는 `memory_retrieve.py`와 `memory_search_engine.py`에 module-level try/except + `e.name` scoping을 추가하는 코드 변경이다. 이 plan의 Actions #1-#4 범위 밖이므로 별도 plan (`plan-import-hardening.md`)으로 추적 필요. 부분 배포 시 hook crash를 방지하는 "Most impactful fix"로 Deep Analysis가 평가한 항목이므로 우선 구현 권장.
