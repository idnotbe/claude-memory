...
**날짜:** 2026-02-22
**범위:** 4개 PoC 실험 (Agent Hook, BM25 정밀도, OR-query 정밀도, Nudge 준수율)
...
| 파일 | 라인 | PoC 관련 |
|------|------|---------|
| `hooks/hooks.json` | 54-66 | #4 -- 현재 UserPromptSubmit hook (command type) |
...
**목적:** 현재 BM25 검색의 precision@k와 recall@k를 정량화. Action #1 (절대 하한선; 클러스터 감지는 Finding #2에서 dead code로 확인됨) 사전/사후 비교 baseline 확보. precision@k는 BM25 일반 품질 baseline이며, Action #1 전후 비교 지표는 **label_precision** (ranking 불변이므로 precision@k는 동일).
...
> **V2-adversarial + Deep Analysis 최종 반영 -- Score Domain 및 측정 방법론:**
>
...
>
> **Triple-field 로깅:** Plan #2에서 결과당 3개 점수 필드 기록: `raw_bm25` (순수 BM25), `score` (복합 점수 = BM25 - body_bonus), `body_bonus`. 이를 통해 두 점수 도메인에서 각각 분석 가능.
>
...
> - **Dual precision 계산:** 두 도메인 모두에서 precision 계산하여 body_bonus의 랭킹 기여도 분석
>
> **Action #1 사전/사후 비교 -- label_precision 지표 (Deep Analysis 반영):**
> Action #1은 `confidence_label()`만 변경하며 ranking은 불변. 따라서 **precision@k 값은 Action #1 전후 동일한 것이 정상** (null result가 아님). 측정 대상은 "ranking quality"가 아니라 **"label quality" (label_precision)**:
> ```
> label_precision_high = count(labeled "high" AND relevant) / count(labeled "high")
> ```
> 이 지표는 "high" 라벨이 실제로 관련 있는 결과에만 부여되는지를 측정.
>
> **관련성 ground truth:** 큐레이션된 쿼리셋에 대한 **인간 어노테이션**으로 판정.
> 어노테이터는 각 결과를 검토하고 루브릭에 따라 relevant/irrelevant 마킹:
> "이 메모리를 보고 Claude가 더 나은 답변을 할 수 있는가?"
...
4. **메트릭 계산:** precision@3, precision@5, recall@k (recall@k 한계 -- 아래 주석 참조)

> **⚠ 평가 방법론 한계 -- recall@k (V2 검증 발견):**
> recall@k의 분모(전체 relevant 문서 수)를 확정하려면 전체 코퍼스에 대한 완전한 어노테이션이 필요하다. 메모리 수가 수백 건 이상으로 성장하면 이는 비현실적.
>
> **대안 후보:**
> 1. **Stratified sampling + Horvitz-Thompson 보정** -- 코퍼스를 계층 샘플링하여 recall 추정
> 2. **Inferred Average Precision (infAP)** -- 불완전 판정(incomplete judgments)에서 AP를 추정하는 표준 IR 기법
> 3. **Pooling** -- 여러 검색 전략의 top-k 합집합만 어노테이션 (TREC 방식)
> 4. **recall@k 제외** -- precision@k만으로 평가, recall은 측정하지 않음
>
> **처분:** Phase A 파일럿에서 추정 기법 선정을 **필수 산출물**로 포함한다. 파일럿 데이터의 코퍼스 규모와 어노테이션 비용을 보고 적절한 대안을 확정.
...
> **⚠ 평가 방법론 한계 -- Phase B 성공 기준 (V2 검증 발견):**
> 현재 Phase B에 정량적 성공/실패 기준이 없다. Baseline 없이 임계치를 사전 설정하는 것은 자의적이므로, Phase A가 baseline을 제공한 후에야 확정할 수 있다.
>
> **예비 프레임워크 (Phase A baseline 확보 후 확정):**
> - **통계 검정:** Paired bootstrap confidence interval (95%)
> - **최소 효과 크기:** 절대 개선 5% 이상 (예: precision@3 기준 60% → 65%)
> - **보고 형식:** Effect size + confidence interval **필수** 보고. CI 폭이 10%를 초과하면 샘플 추가 수집
>
> **처분:** Phase A 파일럿 완료 시 baseline 수치를 기반으로 Phase B 성공 기준을 사전 확정한다. 이 프레임워크는 예비안이며, 파일럿 데이터의 분산(variance)에 따라 조정될 수 있다.
...
> **⚠ 평가 방법론 한계 -- Prefix-match 과소 계수 (V2 검증 발견):**
> 위 코드의 `query_tokens & result_tokens` 정확 교집합(set intersection)은 FTS5의 prefix wildcard 매칭을 반영하지 못한다.
> ```
> 예: query="auth" → FTS5: "auth"* → "authentication" 매칭
>     하지만 {"auth"} & {"authentication"} = ∅ → matched_token_count = 0
> ```
> 이로 인해 `polluted_query_rate`가 **과대 추정**될 수 있다 (실제로는 접두사 매칭으로 연결된 결과를 "무관"으로 오분류).
>
> **수정:** `startswith()` 접두사 매칭으로 교체:
> ```python
> matched_tokens = {qt for qt in query_tokens
>                   if any(rt.startswith(qt) for rt in result_tokens)}
> ```
>
> **Plan #2 연계:** `matched_tokens` 로깅 필드(Plan #2 `retrieval.search` 스키마)에도 동일한 접두사 매칭 로직을 적용해야 한다. 정확 교집합 기반 로깅은 동일한 과소 계수 문제를 로그 데이터에 전파시킨다.
...
### PoC #6: Nudge 준수율 측정 (**PARTIALLY UNBLOCKED** -- Deep Analysis 반영)

> **V2-adversarial HIGH 반영 -- Dead Correlation Path:**
> `/memory:search` skill은 `memory_search_engine.py` CLI를 호출. CLI 모드에는 `hook_input`이 없으므로 `session_id`가 빈 문자열. `retrieval.inject.session_id`와 `search.query.session_id` 조인은 **구조적으로 0 매칭**을 반환.
>
> **Deep Analysis 해결 (Finding #4, ~12 LOC):**
> 1. `memory_search_engine.py`에 `--session-id` argparse 파라미터 추가 (optional, default 빈 문자열)
> 2. 우선순위: `CLI arg > CLAUDE_SESSION_ID env var > 빈 문자열`
> 3. 결과 출력 후(L495 이후) `emit_event("search.query", ...)` 호출 시 `session_id` 전달
> 4. SKILL.md 변경 없음 (LLM은 session_id에 접근 불가)
>
> **현재 상태:** `--session-id` CLI 파라미터로 **수동 상관관계** 가능. 자동 skill-to-hook 상관관계는 `CLAUDE_SESSION_ID` env var 지원 대기.
> 수동 상관관계만으로도 탐색적 데이터 수집 범위에서 충분.
...
- [ ] recall@k 추정 기법 선정 (stratified sampling, infAP, pooling, 또는 recall 제외 중 결정)
- [ ] Phase B 성공 기준 사전 확정 (baseline 기반 paired bootstrap CI + 최소 효과 크기)
- [ ] 확장 쿼리셋 수집 (50개 이상)
...
- [ ] 각 결과의 토큰 매칭 정보 추출 (title+tags 토큰화 → 쿼리 토큰 **접두사 매칭**)
...
| recall@k | #5 | 전체 relevant 중 top-k에 포함된 비율 | recall 손실 없이 precision 개선 확인 (⚠ 분모 확정에 전체 코퍼스 어노테이션 필요 -- Phase A에서 추정 기법 확정) |
...
| 검토 | 결과 | 핵심 발견 |
|------|------|----------|
| Engineering Review | APPROVE WITH CHANGES | PoC #5 sample size, recall@k limitation |
| Adversarial Review | APPROVE WITH CHANGES | PoC #6 methodology reclassified to exploratory |
| V1-Robustness | PASS WITH NOTES | Test-retest reliability, pilot verification criteria |
| V1-Practical | PASS | Tools available, methodology feasible |
| V2-Fresh Eyes | APPROVE | PoC #5 methodology needs tightening (CI width, recall@k) |
| V2-Adversarial | HIGH → fixed | PoC #5 score domain paradox, PoC #6 dead correlation path (BLOCKED) |
...
| V2 Evaluation Methodology | 3 findings incorporated | recall@k 분모 한계 + 추정 기법 대안, prefix-match 과소 계수 수정, Phase B 성공 기준 프레임워크 추가 |
